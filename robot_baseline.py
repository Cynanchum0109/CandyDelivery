...
# -*- coding: utf-8 -*-
"""
Robot Baseline - Uses MLP model to predict interaction time and auto-stop
Based on robot_main.py, but without video recording, only timing
"""
...
from baseline_model import (
    load_trained_model, predict_end_time, 
    ...
    def run(self, timeout=None):
        ...
        try:
            ...
            if self.approach_start_time and self.approach_duration == 0.0:
                self.approach_duration = time.time() - self.approach_start_time
            # Keep resources for continued use
...
class VoiceCandyChatWithTimeout:
    """VoiceCandyChat wrapper with timeout functionality"""
    
    def __init__(self, voice_chat, timeout_duration):
        """
        Args:
            voice_chat: VoiceCandyChat instance
    ...
    def _timeout_monitor(self):
        ...
        while not self.should_stop.is_set():
            ...
            if remaining <= 0:
                print(f"\n{'='*60}")
                print(f"⏰ Predicted interaction duration reached ({self.timeout_duration:.1f} seconds)")
                print(f"{'='*60}")
            ...
            # Check more frequently in last 10 seconds (every 1 second)
            if remaining <= 10:
                print(f"⏳ Interaction remaining time: {remaining:.1f}s (will auto-end soon)")
                time.sleep(1.0)
            # Remind every 3 seconds in last 30 seconds
            elif remaining <= 30:
                if int(remaining) % 3 == 0:
                    print(f"⏳ Interaction remaining time: {remaining:.1f}s (total: {self.timeout_duration:.1f}s)")
                time.sleep(1.0)
            # Print remaining time every 5 seconds
            elif int(elapsed) % 5 == 0 and elapsed > 0:
                print(f"⏳ Interaction remaining time: {remaining:.1f}s (total: {self.timeout_duration:.1f}s)")
                time.sleep(0.5)
            else:
                time.sleep(0.5)
    ...
    def run_with_timeout(self):
        """Run conversation and auto-stop when timeout is reached"""
...
def main():
    """Main program flow."""
    print("=" * 60)
    print("Candy Delivery Robot - Baseline Mode (with MLP Prediction)")
    print("=" * 60)
    print()
    
    # Load prediction model
    print("Loading interaction time prediction model...")
    try:
    ...
    # Start timing (replaces video recording)
    session_start_time = time.time()
    print(f"⏱️  Timing started: {datetime.now().strftime('%H:%M:%S')}")
    print()
    ...
    try:
        ...
        # Step 2: Voice Conversation (with prediction-based timeout)
        print("\n" + "=" * 60)
        print("Starting Voice Conversation System...")
        print("=" * 60)
        print()
        
        # Record conversation start time (relative to session start)
        conversation_start_time = time.time()
        start_time_offset = conversation_start_time - session_start_time
        
        start_time_str = seconds_to_time_str(start_time_offset)
        print(f"📝 Conversation start time (relative to timing start): {start_time_str}")
        
        # Use model to predict interaction duration
        predicted_duration = None
        if model and scaler:
            try:
                start_time_seconds = start_time_offset
                predicted_end_time = predict_end_time(
                    start_time_seconds, 
                    model, 
                    scaler, 
                    durations_history
                ...
                predicted_duration = predicted_end_time - start_time_seconds
                ...
                import traceback
                traceback.print_exc()
                predicted_duration = 120.0  # Default 120 seconds
        else:
            predicted_duration = 120.0  # Default 120 seconds
            print(f"⚠ Using default interaction duration: {predicted_duration}s")
